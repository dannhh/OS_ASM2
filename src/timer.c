...
//  Update time_slot, lock event_lock(cond) and wait for signal
static void * timer_routine(void * args) {
	while (!timer_stop) {
		printf("Time slot %3lu\n", current_time());
		int fsh = 0; // Count finish event
		int event = 0;
		...
		for (temp = dev_list; temp != NULL; temp = temp->next) {
			...
			temp->id.done = 0;
			pthread_cond_signal(&temp->id.timer_cond); // Wake up
...
// Alloc container and init cond, mutex, 
// add new container to the front of dev_list
