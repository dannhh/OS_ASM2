...
// Allocate n pages _mem_stat and store first byte address to reg_index register.
...
// Free the memory which are allocated in reg_index register.
static int free_data(struct pcb_t * proc, uint32_t reg_index) {
	return free_mem(proc->regs[reg_index], proc);
}

// Read data from (address of destination register + offset).
static int read(
...
// Write data to (address of destination register + offset).
static int write(
		struct pcb_t * proc, // Process executing the instruction
